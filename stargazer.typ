...
#let stargazer-theme(
  ...
  show: touying-slides.with(
    config-page(
      ...
      header-ascent: 0em,
      footer-descent: 0em,
      margin: (top: 3.7em, bottom: 1.2em, x: 2.5em),
    ),
    config-common(
      slide-fn: slide,
    ...
    config-store(
      ...
      header: self => if self.store.title != none {
        block(
          ...
          place(left + horizon, text(fill: self.colors.neutral-lightest, weight: "bold", size: 1.0em, utils.call-or-display(self, self.store.title)), dx: 1.5em),
        )
      },
      footer: self => {
