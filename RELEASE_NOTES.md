...
This update keeps `Slide-DoAnHTN-Nhom17-DE10Standard` aligned with the English-facing engineering portfolio while removing dash-line motion from SVG visuals.

## Changes

- Cleaned `assets/portfolio-motion.svg` so SVG motion no longer uses moving dashed or dotted line effects.
- Cleaned `docs/banner.svg` so SVG motion no longer uses moving dashed or dotted line effects.
